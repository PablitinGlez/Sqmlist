<div>
    ...
    <p class="text-gray-600 mb-4 text-left text-xs sm:text-sm">Envía tus datos</p>

    @if (session()->has('success'))
        <div class="mb-3 font-medium text-xs sm:text-sm text-green-600 bg-green-50 p-2 sm:p-3 rounded-md">
            {{ session('success') }}
        </div>
    @endif

    @if (session()->has('error'))
        <div class="mb-3 font-medium text-xs sm:text-sm text-red-600 bg-red-50 p-2 sm:p-3 rounded-md">
            {{ session('error') }}
        </div>
    @endif
    ...
    <form wire:submit.prevent="submitForm" class="space-y-3 sm:space-y-4">
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
            <div>
                <input
                    id="name"
                    class="block mt-1 w-full p-2 sm:p-3 border border-gray-300 rounded-md focus:ring-blue-500 focus:border-blue-500 text-xs sm:text-sm"
                    type="text"
        ...
        <div>
            <textarea
                ...
                rows="3"
                class="block mt-1 w-full p-2 sm:p-3 border border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 resize-none text-xs sm:text-sm"
        ...
        <div class="mt-4 sm:mt-6">
            <button
                type="submit"
                class="w-full inline-flex items-center justify-center py-2 sm:py-2.5 text-xs sm:text-sm bg-blue-500 hover:bg-blue-600 text-white font-semibold rounded-md shadow-sm transition-colors duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
            ...
            >
                ...
                <span wire:loading wire:target="submitForm" class="inline-flex items-center justify-center">
                    ...
                    <span class="inline-block text-xs sm:text-sm">Enviando...</span>
                </span>
            </button>
        </div>
