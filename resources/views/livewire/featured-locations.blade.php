...
<section class="py-16 bg-white">
    <x-partials.container>
        ...
        <div class="flex flex-wrap justify-center gap-2 md:gap-4 mb-8 border-b">
            @foreach ($locationsData as $key => $data)
                <button
                    wire:click="selectTab('{{ $key }}')"
                    class="px-4 py-2 text-sm md:text-base font-semibold rounded-t-lg transition-colors duration-200
        ...
        <div class="p-4  rounded-lg ">
            @if (isset($locationsData[$activeTab]))
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    {{-- Sección de Venta --}}
                    <div>
                        <h3 class="text-2xl font-bold text-gray-800 mb-4">Venta</h3>
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            @foreach ($locationsData[$activeTab]['venta'] as $locationName => $propertyTypes)
                                <div>
                                    <h4 class="font-semibold text-gray-700 mb-2">{{ $locationName }}</h4>
                                    <ul class="space-y-1">
                                        @foreach ($propertyTypes as $fullText)
                                            ...
                                            <li>
                                                <a href="{{ $this->generateUrl($locationName, $propertyType, 'venta') }}"
                                                   class="text-sm text-blue-600 hover:text-blue-800 hover:underline" wire:navigate>
                                                    {{ $fullText }}
                                                </a>
                                            </li>
                                        @endforeach
                    ...
                    {{-- Sección de Renta --}}
                    <div>
                        <h3 class="text-2xl font-bold text-gray-800 mb-4">Renta</h3>
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            @else
                <p class="text-center text-gray-600">No hay datos disponibles para esta sección.</p>
            @endif
        </div>
    </x-partials.container>
</section>
